...
export default function ContentManager() {
  ...
  const [activeTab, setActiveTab] = useState<'press' | 'parliament'>('press')
  ...
  const [formType, setFormType] = useState<'pressRelease' | 'parliamentaryUpdate'>('pressRelease')
  ...
  const handleOpenCreate = (type: 'pressRelease' | 'parliamentaryUpdate') => {
    setFormType(type)
  ...
  const handleOpenEdit = (item: any, type: 'pressRelease' | 'parliamentaryUpdate') => {
    setFormType(type)
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <button
            onClick={() => handleOpenCreate(activeTab === 'press' ? 'pressRelease' : 'parliamentaryUpdate')}
          ...
          >
            ...
            <span>Create {activeTab === 'press' ? 'Press Release' : 'Parliamentary Update'}</span>
          </button>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm mb-6 flex items-center">
          <div className="relative w-full">
            <input
              ...
              placeholder={`Search ${activeTab === 'press' ? 'Press Releases' : 'Parliamentary Updates'} by title or text...`}
            />
            ...
            >
              ...
              <div className="px-6 py-5 bg-navy-900 text-white flex justify-between items-center sticky top-0 z-10 border-b border-navy-950">
                <div>
                  <span className="block text-[9px] text-slate-400 font-bold uppercase tracking-widest">
                    {formType === 'pressRelease' ? 'Press Release Document' : 'Parliamentary Update Document'}
                  </span>
              ...
              <form onSubmit={handleSubmitForm} className="p-6 flex-grow space-y-6 text-left">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">
                      {formType === 'pressRelease' ? 'Publication Date *' : 'Session Date *'}
                    </label>
                    <input
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">
                    {formType === 'pressRelease' ? 'Short Excerpt / Intro (Plain Text)' : 'Summary Description *'}
                  </label>
                  <textarea
                    rows={formType === 'pressRelease' ? 2 : 4}
                    required={formType === 'parliamentaryUpdate'}
                    ...
                    placeholder={formType === 'pressRelease' ? 'Provide a brief summary for lists pages...' : 'Enter the complete update summary...'}
                  />
                </div>

                {/* Body Content (Press Release only) */}
                {formType === 'pressRelease' && (
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Enter the main body of the press release. Separate paragraphs with double newlines (press enter twice). This will automatically format to Sanity blocks."
                    />
                  </div>
                )}
