...
interface DailyUpdatesClientProps {
...
export default function DailyUpdatesClient({ dailyUpdates, activeId }: DailyUpdatesClientProps & { activeId?: string | null }) {
  ...
  const readableText = activeContent
    ...
    : ''
  const { speak, stop, state: ttsState, supported: ttsSupported, currentCharIndex } = useTextToSpeech(readableText, language)
...
━━━━━━━━━━━━━━━━━━` : ''
  ...
  }, [activeContent, isClient, dailyUpdates])
  ...
  const groups: Record<string, { label: string; year: number; month: number; items: DailyUpdateItem[] }> = {}
    ...
    <div className="py-16 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto px-0 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16 relative pb-4 px-4 sm:px-0">
          <span className="text-xs font-bold text-saffron-600 tracking-widest uppercase block mb-2">Activities & Feed</span>
        ...
        {sortedGroupKeys.length > 0 ? (
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <span className="inline-flex items-center text-xs font-bold text-saffron-600 group-hover:text-saffron-700 transition-colors mt-auto pt-2">
                                    <BookOpen className="w-3.5 h-3.5 mr-1" /> Read full details <ChevronRight className="w-3.5 h-3.5 ml-0.5" />
                                  </span>
                                </div>
                              )
                            })}
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-sm">
            ...
            <p className="text-slate-500 text-sm">No daily updates published yet.</p>
          </div>
        )}
          ...
          <motion.div data-modal="true" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setActiveMedia(null)} className="fixed inset-0 z-[60] flex flex-col items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
            <div className="w-full max-w-5xl flex items-center justify-between mb-3 px-1" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => window.open(activeMedia.src, '_blank')} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-saffron-500 hover:bg-saffron-400 text-navy-900 text-xs font-bold transition-colors shadow-lg cursor-pointer" aria-label="View original"><Download className="w-4 h-4" />View Original</button>
